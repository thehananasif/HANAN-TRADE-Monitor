...
export type GlobalTenderFilters = Partial<Pick<ListGlobalTendersRequest,
  'country' | 'countries' | 'region' | 'source' | 'status' | 'deadlineFrom' | 'deadlineTo' | 'minValue' | 'maxValue' | 'currency' | 'category' | 'query' | 'pageSize' | 'cursor' | 'sort' | 'buyer' | 'publishedFrom' | 'publishedTo'>>;
...
export async function fetchGlobalTenders(filters: GlobalTenderFilters = {}): Promise<ListGlobalTendersResponse> {
  const request: ListGlobalTendersRequest = {
    ...
    currency: '', category: '', query: '', pageSize: 25, cursor: '', sort: 'closing_soon', buyer: '', publishedFrom: '', publishedTo: '', ...filters,
  };
