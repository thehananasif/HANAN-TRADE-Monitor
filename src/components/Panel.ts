...
export class Panel {
  ...
  public showGatedCta(reason: PanelGateReason, _onAction: () => void): void {
    // HANAN TRADE Monitor: gated premium panels are hidden entirely instead
    // of rendering a "Sign In to Unlock" / "Upgrade to Pro" CTA. unlockPanel()
    // restores visibility if the user later gains access.
    if (reason === PanelGateReason.NONE) return;
