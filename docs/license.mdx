...
This page is a plain-language summary for contributors and users. It is not legal advice and does not replace the license text in [`LICENSE`](https://github.com/thehananasif/HANAN-TRADE-Monitor/blob/main/LICENSE).

**Exception — official client packages are MIT.** The thin API clients — the [`worldmonitor` npm CLI](https://www.npmjs.com/package/worldmonitor) (`cli/`), and the [Python](https://pypi.org/project/worldmonitor-sdk/) (`sdk/python/`), [Ruby](https://rubygems.org/gems/worldmonitor) (`sdk/ruby/`), and [Go](https://pkg.go.dev/github.com/thehananasif/HANAN-TRADE-Monitor/sdk/go) (`sdk/go/`) SDKs — are licensed under MIT (see the `LICENSE` file in each directory) so they can be embedded in any application without copyleft obligations. This exception covers only those client packages: the World Monitor platform, server, and dashboard remain AGPL-3.0-only.
...
Contact the maintainer through the [GitHub repository](https://github.com/thehananasif/HANAN-TRADE-Monitor) to discuss commercial licensing.
