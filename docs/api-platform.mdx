...
Returns the latest **GitHub Release** of `thehananasif/HANAN-TRADE-Monitor`. Used by the desktop app to detect a newer published release and prompt the user to update. It is **not** the currently-deployed Vercel commit.

```json
{
  "version": "2.6.7",
  "tag": "v2.6.7",
  "url": "https://github.com/thehananasif/HANAN-TRADE-Monitor/releases/tag/v2.6.7",
  "prerelease": false
}
```
...
Redirects to the matching asset on the latest GitHub release of `thehananasif/HANAN-TRADE-Monitor`. Returns `302` to the asset URL on success, or `302` to [releases/latest](https://github.com/thehananasif/HANAN-TRADE-Monitor/releases/latest) on any failure (unknown platform, no match, GitHub error).
