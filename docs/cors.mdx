...
| `worldmonitor-*-user-*.vercel.app` | Vercel preview deploys |
...
Browser-visible response headers include `Mcp-Session-Id`, `WWW-Authenticate`, and `Retry-After` via `Access-Control-Expose-Headers` so MCP clients can continue sessions, re-authenticate, and respect backoff hints.
