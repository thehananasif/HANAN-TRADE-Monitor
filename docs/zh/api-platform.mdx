...
返回 `thehananasif/HANAN-TRADE-Monitor` 的最新 **GitHub Release**。桌面应用使用此端点检测是否有更新的发布版本,并提示用户升级。它**不是**当前部署的 Vercel commit。
...
重定向到 `thehananasif/HANAN-TRADE-Monitor` 最新 GitHub release 中匹配的资产。成功时返回 `302` 指向资产 URL,任何失败时(未知平台、无匹配、GitHub 错误)返回 `302` 指向 [releases/latest](https://github.com/thehananasif/HANAN-TRADE-Monitor/releases/latest)。
