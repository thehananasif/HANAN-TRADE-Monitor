...
<Info>
结账不会为你创建可复用的密钥。**API Starter 和 API Business** 订阅用户在登录后自行创建密钥。**Dashboard Pro 不包含手动 API 密钥**；Pro 用户可以使用已登录的网页版仪表盘，并改用 OAuth 连接受支持的 MCP 客户端。
</Info>

## 创建密钥

1. 打开 [World Monitor 仪表盘](https://worldmonitor.app/dashboard)，使用拥有该 API 订阅的账户登录。
...
手动密钥需要有效的 **API Starter** 或 **API Business** 订阅。Dashboard Pro 订阅可解锁仪表盘和 MCP 功能，但不包含手动创建 API 密钥。
