...
World Monitor 在四个语言生态中提供官方客户端库。它们都是**零依赖**、MCP 优先的 [`worldmonitor` npm CLI](/zh/cli) 镜像：[MCP 服务器](/zh/mcp-overview)是实时且有文档的智能体接口（`tools/list` 公开；`tools/call` 需要用户 API 密钥），此外每个 SDK 都配有一个小型 REST 备用通道，用于主机相对路径和自托管场景。

| 语言 | 软件包 | 安装 | 源码 |
| --- | --- | --- | --- |
| Python | [PyPI 上的 `worldmonitor-sdk`](https://pypi.org/project/worldmonitor-sdk/) | `pip install worldmonitor-sdk` | [`sdk/python/`](https://github.com/thehananasif/HANAN-TRADE-Monitor/tree/main/sdk/python) |
| Ruby | [RubyGems 上的 `worldmonitor`](https://rubygems.org/gems/worldmonitor) | `gem install worldmonitor` | [`sdk/ruby/`](https://github.com/thehananasif/HANAN-TRADE-Monitor/tree/main/sdk/ruby) |
| Go | [pkg.go.dev 上的 `github.com/thehananasif/HANAN-TRADE-Monitor/sdk/go`](https://pkg.go.dev/github.com/thehananasif/HANAN-TRADE-Monitor/sdk/go) | `go get github.com/thehananasif/HANAN-TRADE-Monitor/sdk/go` | [`sdk/go/`](https://github.com/thehananasif/HANAN-TRADE-Monitor/tree/main/sdk/go) |
| JavaScript / CLI | [npm 上的 `worldmonitor`](https://www.npmjs.com/package/worldmonitor) | `npm install worldmonitor` | [`cli/`](https://github.com/thehananasif/HANAN-TRADE-Monitor/tree/main/cli) |
...
数据调用（`tools/call`）需要用户 API 密钥——在 [worldmonitor.app/pro](https://www.worldmonitor.app/pro) 生成一个。有关密钥、OAuth 和浏览器会话的区别，请参见[身份验证](/zh/usage-auth)；各套餐的额度请参见[速率限制](/zh/usage-rate-limits)。
